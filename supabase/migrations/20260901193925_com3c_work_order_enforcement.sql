-- COM-3C: work-order quota enforcement for online/offline individual maintenance.
-- Batch remains outside commercial enforcement; only its direct table bypass is sealed.

create or replace function private.assert_can_create_resource(
  p_organization_id uuid, p_resource text, p_requested integer default 1
) returns void language plpgsql security definer set search_path = '' as $$
declare
  entitlements jsonb; usage jsonb; limit_value integer; usage_value integer;
  resource_key text := case p_resource
    when 'customer' then 'customers' when 'equipment' then 'equipment'
    when 'qr_code' then 'qr_codes' when 'quote' then 'quotes_monthly'
    when 'work_order' then 'work_orders_monthly' end;
begin
  if p_requested < 1 or resource_key is null then raise exception 'Invalid commercial resource' using errcode = '22023'; end if;
  ...
  limit_value := (entitlements->'limits'->>resource_key)::integer;
  usage_value := (usage->>resource_key)::integer;
  if limit_value is not null and usage_value + p_requested > limit_value then
    raise exception using message = 'PLAN_LIMIT_REACHED', detail = jsonb_build_object(
      ...
      'limit',limit_value,'requested',p_requested,'plan_code',entitlements->>'plan_code'
    )::text, errcode = 'P0001';
  end if;
end;
$$;

create or replace function public.create_work_order_offline(payload jsonb)
...
declare
  ...
  v_order_id uuid; item jsonb; technical_check jsonb; measurement jsonb;
  subtotal numeric(14,2) := 0; existing_order uuid;
begin
  ...
  for item in select value || jsonb_build_object('_kind','service') from jsonb_array_elements(coalesce(payload->'services','[]'::jsonb))
    ...
    values (v_organization_id,v_order_id,nullif(item->>'id','')::uuid,(item->>'_kind')::public.catalog_item_kind,item->>'name',nullif(item->>'description',''),coalesce((item->>'quantity')::numeric,1),coalesce((item->>'price')::numeric,0));
  end loop;
  for technical_check in select value from jsonb_array_elements(coalesce(payload->'checks','[]'::jsonb)) loop
    ...
    values (v_organization_id,v_order_id,technical_check->>'key',technical_check->>'label',nullif(technical_check->>'category',''),(technical_check->>'status')::public.technical_check_status,nullif(technical_check->>'observation',''),coalesce((technical_check->>'order')::integer,0));
  end loop;
  for measurement in select value from jsonb_array_elements(coalesce(payload->'measurements','[]'::jsonb)) loop
    ...
    values (v_organization_id,v_order_id,measurement->>'key',measurement->>'label',(measurement->>'value')::numeric,measurement->>'unit',coalesce(measurement->>'source','manual'),coalesce((measurement->>'order')::integer,0));
  end loop;
  return v_order_id;
exception when unique_violation then
  select id into v_order_id from public.work_orders where organization_id=v_organization_id and offline_local_id=v_local_id;
