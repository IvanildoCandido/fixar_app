...
export function EmptyState({ title, description }: { title: string; description?: string }) { const theme = useTheme(); return <StateRoot><AlertCircle size={28} color={theme.colors.muted} /><StateTitle>{title}</StateTitle>{description ? <StateDescription>{description}</StateDescription> : null}</StateRoot>; }
export function ErrorState({ title = "Não foi possível carregar", description }: { title?: string; description?: string }) { const theme = useTheme(); return <StateRoot accessibilityLiveRegion="polite"><TriangleAlert size={28} color={theme.colors.danger} /><StateTitle>{title}</StateTitle>{description ? <StateDescription>{description}</StateDescription> : null}</StateRoot>; }
