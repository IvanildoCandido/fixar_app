...
export const generateMultipleHtml = (repairs: Repair[], period: Period, company: ReportOrganization) => {
  ...
  const summary = `<section class="report-card"><h3>${reportIcon("note")}Resumo do período</h3><div class="report-field-grid">${reportField("Atendimentos", repairs.length)}${reportField("Período", periodLabel)}${reportField("Valor total", money(repairs.reduce((sum, repair) => sum + Number(repair.total || 0), 0)), true)}</div></section>`;
