...
export const generateBudgetsHtml = (services: servicesTotal[], parts: servicesTotal[], total: number, company: ReportOrganization, customer: Customer, comments?: string) => {
  ...
  const valueCard = `<section class="report-card"><h3>${reportIcon("money")}Valor total da proposta</h3><div class="report-total">${money(total)}</div><div class="proposal-status">Aguardando aprovação</div></section>`;
